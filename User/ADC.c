...
POINT_t const DACCAL[DAC_CAL_POINT]={ 
                                    {0,0},
                                    {300,1.126},
                                    {600,2.273},
                                    {900,3.420},
...
static const u16 sensor_timer[SENSOR_TIME_CNT] = {4,20,40,80,120,200,400};
static const uint16_t B57164CalPoint[11][2] = {
                                    {50,K10*0.33363},
                                    {45,K10*0.41026},
                                    {40,K10*0.5074},
                                    {35,K10*0.63268},
...
static uint8_t ADC2_CHANNEL[DC_CHANNEL] = {  ADC_CH_2, ADC_CH_5,ADC_CH_6,ADC_CH_7,ADC_CH_14,ADC_CH_15, ADC_CH_8};
static int16_t sens_press=0;
static int16_t sens_press1=0;

static ADC_Conversionl_Buf_t DataBuffer[DC_CHANNEL+4 ];
static u8 cla_zero_end = 0;
static u8 calibration_zero_flag = 0;
static u8 calibration_zero_count = 0;
...
void vDataBufferInit(void)
{
    uint8_t _sens_cnt = (getReg8(SENSOR_COUNT) < SENSOR_TIME_CNT) ? getReg8(SENSOR_COUNT) : 0;
    DataBuffer[0].ConversionalSize = sensor_timer[_sens_cnt];
    DataBuffer[1].ConversionalSize = sensor_timer[_sens_cnt];
...
/**
*  @brief §¶§è§ß§Ü§è§Ú§ñ §Ü§à§ß§Ö§é§ß§à§Ô§à §Ñ§Ó§ä§à§Þ§Ñ §à§á§â§à§ã§Ñ §Õ§Ñ§ä§é§Ú§Ü§Ñ §Õ§Ñ§Ó§Ý§Ö§ß§Ú§ñ
*  
*  @param channel, §Ü§Ñ§Ü§ß§Ñ§Ý I2C
*  @param SENS_FSM, §å§Ü§Ñ§Ù§Ñ§ä§Ö§Ý§î §ß§Ñ §á§Ö§â§Ö§Þ§Ö§ß§ß§å§ð §Ü§à§ß§Ö§é§ß§à§Ô§à §Ñ§Ó§ä§à§Þ§Ñ§ä§Ñ §Õ§Ñ§ä§é§Ú§Ü§Ñ
*  @param fsm,  §å§Ü§Ñ§Ù§Ñ§ä§Ö§Ý§î §ß§Ñ §á§Ö§â§Ö§Þ§Ö§ß§ß§å§ð §Ü§à§ß§Ö§é§ß§à§Ô§à §Ñ§Ó§ä§à§Þ§Ñ§ä§Ñ §Ú§ä§Ö§â§æ§Ö§Û§ã§Ñ
*  @param sens_press, §å§Ü§Ñ§Ù§Ñ§ä§Ö§Ý§î §ß§Ñ §á§Ö§â§Ö§Þ§Ö§ß§ß§å§ð §Õ§Ñ§Ó§Ý§Ö§ß§Ú§ñ
*/
        ...
        case SENSOR_GET_PERS_3:
            ...
            case SENSOR_GET_TEMP_2:
                if ( GetI2CDataFSM(i2c, 0x0A, &i2cdata[index][4],fsm) == 1)
                {
                    ...
                    temperature = (sens_temp & 0x8000) ? (sens_temp - 65536) : sens_temp;                
...
{
    static int16_t calib_data[SENSOR_CNT][CALIB_COUNT];

    if (calibration_zero_count < CALIB_COUNT)
    {
        calib_data[SENSOR1][calibration_zero_count] = sens_press;
        calib_data[SENSOR2][calibration_zero_count] = sens_press1;
    ...
    else
    {
        ...
        {
            temp += calib_data[SENSOR1][i];
            temp1 += calib_data[SENSOR2][i];
        }
        DataBuffer[SENSOR1].offset   = temp  / CALIB_COUNT;
        DataBuffer[SENSOR2].offset   = temp1 / CALIB_COUNT;
        calibration_zero_flag  = 0;
        calibration_zero_count = 0;
        setReg16(SENSOR1_ZERO, DataBuffer[SENSOR1].offset);
        setReg16(SENSOR2_ZERO, DataBuffer[SENSOR2].offset);
        cla_zero_end = 1;
    } 
...
void I2C_task(void *pvParameters)
{
    ...
    while(1)
    {
        ...
        while (1)
        {
            ...
            // §¬§à§ß§Ö§è §è§Ú§Ü§Ý§Ñ §à§á§â§à§ã§Ñ §Õ§Ñ§ä§é§Ú§Ü§à§Ó ( §ß§Ö §Ò§à§Ý§Ö§Ö 25 §Þ§ã )
            if (sensor_time_out > 25)  
            {
                ...
                {
                    PressSens[SENSOR1] = (float)GetConversionali2c(&DataBuffer[0]);    
                }
                ...
                {
                    PressSens[SENSOR2] = (float)GetConversionali2c(&DataBuffer[1]);    
                }
                break;
            }
            else if (sensor_time_out > 5)  // §¦§ã§Ý§Ú §á§â§à§ê§Ý§à §Ò§Ý§à§Ö§Ö 5 §Þ§ã
            {
